...
<html>
  ...
  <html:form action="changeItemIssue" >
		...
		<%}} %>
		<tr>
		<td class="blue">เลขเอกสาร :&nbsp; <html:text property="docCode" ></html:text>
		<html:submit property="reqCode"> 
					<bean:message bundle="inventoryResources" key="change.button.checkDoc" />
 		</html:submit>
 		<%if(request.getAttribute("checkDoc") != null){ %>
		<%= (String) request.getAttribute("checkDoc") == "ok" ? "เลขเอกสารนี้สามารถใช้งานได้" : "<font color=\"red\">ไม่พบเลขเอกสาร</font>" %>
		<%} %>
		</td>
		</tr>
		<br/>
